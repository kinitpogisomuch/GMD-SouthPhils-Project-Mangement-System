...
<body class="page-enter">
    ...
    <main class="admin-content">
            ...
            <div class="emp-tab-content active" id="tab-profile">
                <div class="settings-layout">
                    ...
                    <div class="settings-avatar-card">
                        ...
                        <div class="settings-avatar-name">{{ $client->full_name }}</div>
                        <div class="settings-avatar-role">Client</div>

                        <form method="POST" action="{{ route('client.settings.photo') }}"
                              enctype="multipart/form-data" id="photoUploadForm">
                            @csrf
                            <input type="file" name="profile_photo" id="avatarInput"
                                   accept="image/jpeg,image/png,image/webp" style="display:none;">
                        </form>
                        <form method="POST" action="{{ route('client.settings.photo.remove') }}" id="removePhotoForm" style="display:none;">
                            @csrf
                            @method('DELETE')
                        </form>
                        ...
                        <div class="settings-meta-list">
                            ...
                            <div class="settings-meta-item">
                                ...
                                <span>Member since {{ $client->created_at?->format('Y') }}</span>
                            </div>
                            @if($client->province || $client->city)
                            <div class="settings-meta-item">
                                <i data-lucide="map-pin"></i>
                                <span>{{ implode(', ', array_filter([$client->province, $client->city])) }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div style="flex:1;">
                        <div class="pv-card">
                            ...
                            <form method="POST" action="{{ route('client.settings.profile') }}" id="profileForm">
                                @csrf
                                @method('PUT')
                                <div class="form-grid">
                                    <div class="form-group form-group-full">
                                        <label>Full Name / Company Name</label>
                                        <input type="text" name="full_name" class="profile-field"
                                               value="{{ old('full_name', $client->full_name) }}"
                                               placeholder="e.g. Juan Dela Cruz or ABC Construction Co." disabled>
                                    </div>
                                    <div class="form-group">
                                        <label>Username <span style="font-size:11px;color:var(--muted);font-weight:400;">(cannot be changed)</span></label>
                                        <input type="text" value="{{ $client->username }}" disabled
                                    ...
                                    <div class="form-group">
                                        <label>Account Type <span style="font-size:11px;color:var(--muted);font-weight:400;">(cannot be changed)</span></label>
                                ...
                                <div class="form-grid">
                                    <div class="form-group" id="regionGroup">
                                        ...
                                        <select name="region" id="regionSelect" class="profile-field" disabled>
                                            ...
                                            <option value="{{ old('region', $client->region) }}">{{ old('region', $client->region) }}</option>
                                            @else
                                    ...
                                    <div class="form-group" id="provinceGroup">
                                        ...
                                        <select name="province" id="provinceSelect" class="profile-field" disabled>
                                            @if(old('province', $client->province))
                                            <option value="{{ old('province', $client->province) }}">{{ old('province', $client->province) }}</option>
                                            @else
                                            <option value="">— Not set —</option>
                                            @endif
                                        </select>
                                    ...
                                    <div class="form-group" id="cityGroup">
                                        ...
                                        <select name="city" id="citySelect" class="profile-field" disabled>
                                            @if(old('city', $client->city))
                                            <option value="{{ old('city', $client->city) }}">{{ old('city', $client->city) }}</option>
                                            @else
                                            <option value="">— Not set —</option>
                                            @endif
                                        </select>
            ...
            <div class="emp-tab-content" id="tab-password">
                <div class="pv-card">
                    ...
                    <form method="POST" action="{{ route('client.settings.password') }}" id="passwordForm">
                        @csrf
                        @method('PUT')
    ...
    function saveAvatarCrop() {
        ...
        avatarCropper.getCroppedCanvas({ width: 400, height: 400 }).toBlob(function (blob) {
            ...
            document.getElementById('avatarDisplay').innerHTML =
                '<img src="' + URL.createObjectURL(blob) + '" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">';
    ...
    const SAVED_REGION   = @json($client->region ?? '');
    const SAVED_PROVINCE = @json($client->province ?? '');
    const SAVED_CITY     = @json($client->city ?? '');
    ...
    async function psgcLoadProvinces(regionCode, restore) {
        ...
        try {
            ...
            if (data.length === 0) {
                await psgcLoadCitiesFromRegion(regionCode);
